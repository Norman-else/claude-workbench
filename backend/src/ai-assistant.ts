...
If the user asks about current events, real-time information, or anything requiring up-to-date knowledge, use the web_search tool when available.${projectPath ? `\n\nCurrent project context: The user is working in project "${projectPath}". For all project-scoped tools (MCP, commands, skills, agents), automatically use project_path: "${projectPath}" unless the user explicitly asks for global configuration.` : ''}`;
  ...
  app.get('/api/ai/terminal-confirm-commands', async (_req: Request, res: Response) => {
    try {
  ...
  app.put('/api/ai/terminal-confirm-commands', async (req: Request, res: Response) => {
    try {
      const { userCommands } = req.body as { userCommands: string[] };
 ...
 */
async function getUserConfirmCommands(): Promise<string[]> {
  try {
 ...
 */
async function saveUserConfirmCommands(commands: string[]): Promise<void> {
...
async function requiresConfirmation(command: string): Promise<boolean> {
  ...
  return combined.some(prefix => {
    ...
    if (lowerTrimmed === lowerPrefix) return true;
    if (lowerTrimmed.startsWith(lowerPrefix + ' ')) return true;
    return false;
  });
}
