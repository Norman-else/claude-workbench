...
export function registerAIAssistantRoutes(app) {
    ...
    function modelLabel(baseName, modelId) {
    ...
    app.post('/api/ai/conversations/:id/generate-name', async (req, res) => {
        try {
            const { id } = req.params;
...
async function migrateOldHistory() {
    try {
        ...
        const oldHistory = JSON.parse(oldContent);
...
function redactConversationCredentials(conv) {
    return {
        ...conv,
...
async function handleListPluginAgents(_input) {
    try {
        ...
        for (const plugin of plugins) {
            ...
            for (const pluginAgent of pluginAgents) {
                if (typeof pluginAgent !== 'object' || pluginAgent === null)
                    continue;
                const agentObj = pluginAgent;
...
async function handleGetPluginAgent(input) {
...
async function handleUpdatePluginAgentModel(input) {
    const installPath = input.installPath;
    const filename = input.filename;
 ...
 */
function getDefaultConfirmCommandsForPlatform() {
    ...
    return DEFAULT_CONFIRM_COMMANDS
        ...
        if (entry.platform === 'all')
            return true;
        if (entry.platform === 'unix')
            return !isWin; // unix = macOS + Linux
