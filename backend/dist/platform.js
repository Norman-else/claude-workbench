...
export async function getEnvConfigPath() {
    if (IS_WINDOWS) {
...
async function getPowerShellProfile() {
    try {
...
function buildEnvBlock(vars) {
    const lines = ['', START_MARKER];
    for (const [key, value] of Object.entries(vars)) {
        if (value) {
            lines.push(IS_WINDOWS
                ? `$env:${key} = "${value}"`
                : `export ${key}="${value}"`);
        }
    }
...
function removeEnvBlock(content) {
    return content
        .replace(/# Claude Code Environment Variables - START[\s\S]*?# Claude Code Environment Variables - END\n?/g, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim();
}
/** Read env vars from shell config (Unix) or PowerShell profile (Windows). */
export async function readEnvFromShellConfig() {
    const result = {};
    if (IS_WINDOWS) {
        try {
...
export async function writeEnvToShellConfig(vars) {
    if (IS_WINDOWS) {
        const profilePath = await getPowerShellProfile();
        ...
        for (const key of Object.keys(vars)) {
            content = content.replace(new RegExp(`^\\$env:${key}\\s*=.*$`, 'gm'), '');
        }
...
export async function clearEnvFromShellConfig() {
    if (IS_WINDOWS) {
        try {
            const profilePath = await getPowerShellProfile();
            let content = await fs.readFile(profilePath, 'utf-8');
...
export async function readActiveProfileIdFromShellConfig() {
    if (IS_WINDOWS) {
        try {
            const profilePath = await getPowerShellProfile();
            const content = await fs.readFile(profilePath, 'utf-8');
...
// ============================================================
export async function readSettingsEnv(settingsPath) {
    try {
...
export async function writeSettingsEnv(settingsPath, envVars) {
    let settings = {};
    try {
        const content = await fs.readFile(settingsPath, 'utf-8');
...
export async function clearSettingsEnv(settingsPath) {
    try {
        const content = await fs.readFile(settingsPath, 'utf-8');
        const settings = JSON.parse(content);
        delete settings.env;
...
export function expandPath(inputPath) {
    ...
    if (IS_WINDOWS && p.includes('%')) {
        p = p.replace(/%([^%]+)%/g, (_, envVar) => process.env[envVar] ?? `%${envVar}%`);
    }
